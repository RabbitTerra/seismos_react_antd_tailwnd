...
const trackingSheetSubmitSerializer = (
  ...
  const tsFormData = {
    ...
    active_data: {
      ...
      pre_frac_pulses: {
        pre_frac_start_time: activeDataFormValues.pre_frac_start_time
          ? Number(activeDataFormValues.pre_frac_start_time.format("x"))
          : null,
        pre_frac_end_time: activeDataFormValues.pre_frac_end_time
          ? Number(activeDataFormValues.pre_frac_end_time.format("x"))
          : null,
        pre_frac_num_pulse: activeDataFormValues.pre_frac_num_pulse
          ? Number(activeDataFormValues.pre_frac_num_pulse)
          : null,
      },
      post_frac_pulses: {
        post_frac_start_time: activeDataFormValues.post_frac_start_time
          ? Number(activeDataFormValues.post_frac_start_time.format("x"))
          : null,
        post_frac_end_time: activeDataFormValues.post_frac_end_time
          ? Number(activeDataFormValues.post_frac_end_time.format("x"))
          : null,
        post_frac_num_pulse: activeDataFormValues.post_frac_num_pulse
          ? Number(activeDataFormValues.post_frac_num_pulse)
          : null,
      },
    },
    notes: {
  ...
  tsFormData.stage_tracking = Object.entries(tsFormData.stage_tracking).reduce(
    (a, [k, v]) => (v === null ? a : ((a[k] = v), a)),
    {},
  );
  tsFormData.perforation_interval_information = Object.entries(tsFormData.perforation_interval_information).reduce(
    (a, [k, v]) => (v === null ? a : ((a[k] = v), a)),
    {},
  );
  tsFormData.perforation_interval_information.displacement_volume = Object.entries(
    tsFormData.perforation_interval_information.displacement_volume,
  ).reduce((a, [k, v]) => (v === null ? a : ((a[k] = v), a)), {});
  tsFormData.stage_data = Object.entries(tsFormData.stage_data).reduce(
    (a, [k, v]) => (v === null ? a : ((a[k] = v), a)),
    {},
  );
  tsFormData.stage_data.fluid_parameters = Object.entries(tsFormData.stage_data.fluid_parameters).reduce(
    (a, [k, v]) => (v === null ? a : ((a[k] = v), a)),
    {},
  );
  ...
  tsFormData.stage_data.pumping_summary.max_prop_conc = Object.entries(
    tsFormData.stage_data.pumping_summary.max_prop_conc,
  ).reduce((a, [k, v]) => (v === null ? a : ((a[k] = v), a)), {});
  tsFormData.stage_data.pumping_summary.total_pad_volume = Object.entries(
    tsFormData.stage_data.pumping_summary.total_pad_volume,
  ).reduce((a, [k, v]) => (v === null ? a : ((a[k] = v), a)), {});
  tsFormData.stage_data.pumping_summary.total_clean_fluid_volume = Object.entries(
    tsFormData.stage_data.pumping_summary.total_clean_fluid_volume,
  ).reduce((a, [k, v]) => (v === null ? a : ((a[k] = v), a)), {});
  tsFormData.stage_data.pumping_summary.total_proppant = Object.entries(
    tsFormData.stage_data.pumping_summary.total_proppant,
  ).reduce((a, [k, v]) => (v === null ? a : ((a[k] = v), a)), {});
  tsFormData.stage_data.pumping_summary.acid_volume = Object.entries(
    tsFormData.stage_data.pumping_summary.acid_volume,
  ).reduce((a, [k, v]) => (v === null ? a : ((a[k] = v), a)), {});
  tsFormData.stage_data.pumping_summary.flush_volume = Object.entries(
    tsFormData.stage_data.pumping_summary.flush_volume,
  ).reduce((a, [k, v]) => (v === null ? a : ((a[k] = v), a)), {});
  ...
  tsFormData.active_data.pulsing_parameteres = Object.entries(tsFormData.active_data.pulsing_parameteres).reduce(
    (a, [k, v]) => (v === null ? a : ((a[k] = v), a)),
    {},
  );
  tsFormData.active_data.pre_frac_pulses = Object.entries(tsFormData.active_data.pre_frac_pulses).reduce(
    (a, [k, v]) => (v === null ? a : ((a[k] = v), a)),
    {},
  );
  tsFormData.active_data.post_frac_pulses = Object.entries(tsFormData.active_data.post_frac_pulses).reduce(
    (a, [k, v]) => (v === null ? a : ((a[k] = v), a)),
    {},
  );
...
const trackingSheetPopulateDataSerializer = (trackingSheetData) => {
  ...
  const stageDataValuesData = {
    stage_start_time: trackingSheetData.stage_data.stage_start_time
      ? moment(trackingSheetData.stage_data.stage_start_time)
      : null,
    stage_end_time: trackingSheetData.stage_data.stage_end_time
      ? moment(trackingSheetData.stage_data.stage_end_time)
      : null,
  ...
  const activeDataFormValuesData = {
    ...
    pre_frac_start_time: trackingSheetData.active_data.pre_frac_pulses.pre_frac_start_time
      ? moment(trackingSheetData.active_data.pre_frac_pulses.pre_frac_start_time * 1000)
      : null,
    pre_frac_end_time: trackingSheetData.active_data.pre_frac_pulses.pre_frac_end_time
      ? moment(trackingSheetData.active_data.pre_frac_pulses.pre_frac_end_time * 1000)
      : null,
    pre_frac_num_pulse: trackingSheetData.active_data.pre_frac_pulses.pre_frac_num_pulse,
    post_frac_start_time: trackingSheetData.active_data.post_frac_pulses.post_frac_start_time
      ? moment(trackingSheetData.active_data.post_frac_pulses.post_frac_start_time * 1000)
      : null,
    post_frac_end_time: trackingSheetData.active_data.post_frac_pulses.post_frac_end_time
      ? moment(trackingSheetData.active_data.post_frac_pulses.post_frac_end_time * 1000)
      : null,
